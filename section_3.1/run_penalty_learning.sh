...
do
    mkdir -p penaltyLearning-log/${dataset}
    echo "Rscript run_penalty_learning.R chipseq ${dataset}"
    Rscript run_penalty_learning.R chipseq ${dataset}
done

for dataset in simulated.sin simulated.abs simulated.linear simulated.model.1 simulated.model.2 simulated.model.3
do
    mkdir -p penaltyLearning-log/${dataset}
    echo "Rscript run_penalty_learning.R simulated ${dataset}"
    Rscript run_penalty_learning.R simulated ${dataset}
done
